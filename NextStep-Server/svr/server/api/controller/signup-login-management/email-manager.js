...
module.exports = function (mongoose) {
  ...
  let generateTempUserModel = function (User, callback) {
    ...
    tempUserSchemaObject.createdAt = {
      type: Date,
      //expires: options.expirationTime.toString() + 's', Expiry of Record removed
      default: Date.now
    };
  ...
  // default options
  let options = {
     //verificationURL: apiDetails.email.verificationurl+'account/email-verification/${URL}',
    ...
    verifyMailOptions: {
      ...
      subject: 'Activate your Trueleap account',
    ...
    confirmMailOptions: {
      ...
      html: '<p>Your www.trueleap.io account is now active</p><p>Your temporary password is : ${PASS} , kindly reset it.</p>'
    },
  ...
  let insertTempUser = function (password, tempUserData, callback) {
   ...
   // newTempUser.profile.lastName = tempUserData.profile.lastName !== undefined && tempUserData.profile.lastName.length > 0 ? tempUserData.profile.lastName : "";
    ...
    newTempUser.profile.phoneNumber = tempUserData.profile.phoneNumber !== undefined && tempUserData.profile.phoneNumber.length> 0 ? tempUserData.profile.phoneNumber : "";
    newTempUser.profile.rollNumber = tempUserData.profile.rollNumber !== undefined && tempUserData.profile.rollNumber.length>0 ? tempUserData.profile.rollNumber :"";

    newTempUser.profile.profilephoto = tempUserData.profile.profilephoto !== undefined && tempUserData.profile.profilephoto.length>0 ? tempUserData.profile.profilephoto :"";

    newTempUser.profile.birthDay = tempUserData.profile.birthDay !== undefined && tempUserData.profile.birthDay.length > 0 ? tempUserData.profile.birthDay : "";
    newTempUser.profile.birthMonth = tempUserData.profile.birthMonth !== undefined && tempUserData.profile.birthMonth.length> 0 ? tempUserData.profile.birthMonth : "";
    newTempUser.profile.gurdianName = tempUserData.profile.gurdianName !== undefined && tempUserData.profile.gurdianName.length>0 ? tempUserData.profile.gurdianName :"";


    newTempUser.profile.address={};
    newTempUser.profile.address.zipcode=tempUserData.profile.address.zipcode!== undefined && tempUserData.profile.address.zipcode.length > 0 ? tempUserData.profile.address.zipcode : "";
    ...
    newTempUser.profile.address.apartmentNumber=tempUserData.profile.address.apartmentNumber!== undefined && tempUserData.profile.address.apartmentNumber.length > 0 ? tempUserData.profile.address.apartmentNumber :"";
   ...
   * @param {function} callback - the callback to pass to Nodemailer's transporter
   */
  let sendConfirmationEmail = function (email,password, callback) {
    const logger = getLogger('sendConfirmationEmail');
    let p = /\$\{PASS\}/g;
    //let PASS = options.verificationURL.replace(p, password);

    let mailOptions = JSON.parse(JSON.stringify(options.confirmMailOptions));
    mailOptions.to = email;
    mailOptions.html = mailOptions.html.replace(p, password);
    logger.debug("sendConfirmationEmail mail option is  -" + JSON.stringify(mailOptions));

...
httpsCall.prototype.httpsCall = function (param, fetchData,requestId) {
    ...
    return new Promise(function (resolve, reject) {
        try {
                ...
                (res) => {
                    ...
                    res.on('end', () => {
                        if (res.statusCode >= 200 && res.statusCode <= 299) {
                            ...
                            if (logLevel === "INFO")
                                logger.info("requestId :: " + _requestId + ":: httpsCall request response -" + JSON.parse(body));
                            resolve({ statusCode: res.statusCode, headers: res.headers, body: JSON.parse(body) });
                        }else if(res.statusCode == 400){
                            logger.error("requestId :: " + _requestId + ":: httpsCall request in elseif received -"+ res.statusCode);
            ...
            requ.on('error', ()=>{
                logger.error("requestId :: " + _requestId + ":: httpsCall requ.on(error)");

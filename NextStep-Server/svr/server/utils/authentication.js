...
function hasExpired(created) {
  ...
  var requestId = req.id;
    ...
    //var token=_.hasIn(req.headers,"authorization") && req.headers.authorization!=='undefined' && req.headers.authorization.length>0?req.headers.authorization:req.query.authorization;
      var token=_.hasIn(req.headers,"authorization") && req.headers.authorization!=='undefined' && req.headers.authorization.length>0?req.headers.authorization:"";
        ...
        var queryPayload = {};
            ...
            co(function*() {
                ...
                //var validToken=true;
                if(validToken===true){
                  res.send(mapError.errorCodeToDesc(requestId, '402', "authenticate"));
                }else{
                  if(req.headers.checkuser===authenticateRes[0].category.categoryType)
                    next();
                  else
                    res.send(mapError.errorCodeToDesc(requestId, '403', "authenticate"));
                }
            }).catch(function(err) {

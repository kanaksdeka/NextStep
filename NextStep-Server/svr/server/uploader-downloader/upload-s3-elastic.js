...
const Elastic = require(path.join(uploadappRoot, 'api/models/Elastic'));
...
uploadapp.post('/file-service/upload', authentication, function (req, res) {
    ...
    form.parse(req, function (err, field, files) {
        ...
        grade = uploadparam[0]; // the grade for wwhich the document is uploaded
        uploadtype = uploadparam[1];
        documentid = uploadparam[2]; // this is the period id upload is done for
                    ...
                    if (uploadtype == "AS") { //Start of assignment submission
                        ...
                        async.waterfall([
                            ...
                            if (err) {
                                ...
                                res.status(500).send(mapError.errorCodeToDesc(req.id, '504', "updateperiod"))
                            }
                        }); // End of Async waterfall
                        unlinkrequired = true; // this is an assignment submission so we dont put it in elastic
                    } else { ////End of assignment submission 
                        let periodIndexforElastic = "";
                        async.waterfall([
                            function fetchuser(done) {
                                    ...
                                    function (error, success) {
                                        if (error) {
                                            ...
                                            if (success === null) {
                                                ...
                                                periodIndexforElastic = success.periodindex;
                                                done(null, success)
                            ...
                            function isertforindexing(success, done) {
                                logger.debug("requestId :: " + req.id + ":: Insert for Indexing");
                                //Enter the details for indexing
                                fs.rename(temp_file, '/tmp/' + periodObj.file, (err) => {
                                    if (err) {
                                        logger.error("requestId :: " + req.id + ":: File rename failed due to   -" + err);
                                    } else {
                                        logger.debug("requestId :: " + req.id + ":: File file renamed from  -" + temp_file);
                                        logger.debug("requestId :: " + req.id + ":: File file renamed to -" + '/tmp/' + periodObj.file);
                                    }
                                });
                                let elastic = new Elastic();
                                elastic.filename = periodObj.file;
                                elastic.ec2path = '/tmp/' + periodObj.file;
                                elastic.awsurl = periodObj.path;
                                elastic.filetype = periodObj.file_type;
                                elastic.isindexed = false;
                                elastic.indexPeriod = periodIndexforElastic; //Storing these details to be used to searched document for specific period
                                elastic.period = documentid;
                                elastic.grade = grade;
                                elastic.save((err) => {
                                    if (err) {
                                        logger.error("requestId :: " + req.id + " :: Errorwhile inserting in index", err);
                                    } else {
                                        logger.debug("requestId :: " + req.id + " :: Record for Indexing by Scheduler saved");
                                    }
                                });
                            },
                        ], (err) => {
                            if (err) {
                                logger.debug("requestId :: " + req.id + ":: Async Err -" + err);
                                res.status(500).send(mapError.errorCodeToDesc(req.id, '504', "updateperiod"))
                            }
                        }); // End of Async waterfall
        ...
        .on('error', function (err) {
            ...
            res.status(500).send(mapError.errorCodeToDesc(req.id, '506', "updateperiod"))

        })
        .on('end', function () {
            try {
                if (unlinkrequired == true) {
                    ...
                    logger.debug("requestId :: " + req.id + ":: Reached the end of file upload for - " + temp_file);
                    res.status(200).send(mapError.errorCodeToDesc(req.id, '200', "updateperiod"))
                }
            } catch (err) {
...
uploadapp.post('/file-service/profile-photo/upload', function (req, res) {

    const logger = getLogger('profile-photo');
    ...
    form.parse(req, function (err, field, files) {
        logger.debug("requestId :: " + req.id + ":: Type of field  -" + typeof field);
        logger.debug("requestId :: " + req.id + ":: Field received -" + field.profileid);
                    ...
                    async.waterfall([
                        function updateprofile(done) {
                            User.findOneAndUpdate(
                                ...
                                { _id: profileid },
                                { $set: { "profile.profilephoto": data.Location } },
                                { upsert: true},
                                function (error, success) {
                                    if (error) {
                                        logger.debug("requestId :: " + req.id + ":: Updating profile photo Error -" + error);
                                        unlinkrequired = true;
                                        dbUpdate = false;
                                        done(null, success)
                                    } else {
                                        if (success === null) {
                                            logger.error("requestId :: " + req.id + ":: Updating profile photo error ");
                                        } else {
                                            logger.debug("requestId :: " + req.id + ":: Document Update response received -" + JSON.stringify(success));
                                            unlinkrequired = true;
                                            done(null, success)
                        ...
                        function unlinkfile(success, done) {
                            try {
                                if (unlinkrequired == true) {
                                    logger.debug("requestId :: " + req.id + ":: Trying to delete temp file with profile update success-" + temp_file);
                                    fs.unlink(temp_file, (err) => {
                                        if (err) {  //throw err;
                                            logger.debug("requestId :: " + req.id + ":: Delete of temp file failed -" + temp_file);
                                            res.status(500).send(mapError.errorCodeToDesc(req.id, '505', "updateuser"))
                                    ...
                                    logger.debug("requestId :: " + req.id + ":: Non Reachable Code  - " + temp_file);
                                }
                                logger.debug("requestId :: " + req.id + ":: DB Update flag is set to - " + dbUpdate);

                                if (dbUpdate == false) {
                                    res.status(204).send(mapError.errorCodeToDesc(req.id, '204', "updateuser"))
                                } else {
                                    res.status(200).send(mapError.errorCodeToDesc(req.id, '200', "updateuser"))
                                }
                            } catch (err) {
                                logger.debug("requestId :: " + req.id + ":: Exception while deleting temp file  -" + temp_file);
                                res.status(500).send(JSON.stringify(err))
                        ...
                        if (err) {
                            logger.debug("requestId :: " + req.id + ":: Updated for Profile image Async Err -" + err);
                            res.status(500).send(mapError.errorCodeToDesc(req.id, '504', "updateuser"))
                        }
                    });
                }
            });
        ...
        .on('error', function (err) {
            logger.debug('An error has occured: \n' + err);
            //res.send({ 'success': false, error: err });
            res.status(500).send(mapError.errorCodeToDesc(req.id, '506', "updateuser"))
        ...
        .on('aborted', function () {
            logger.debug("requestId :: " + req.id + ":: Upload aborted by user");
            res.status(500).send(mapError.errorCodeToDesc(req.id, '502', "updateuser"))
        })

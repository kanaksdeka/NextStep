    ...
    if (cluster.isMaster) {
        ...
        if (cpuCount === 1) {
            ...
            server = require('https').createServer({
                ...
                cert: fs.readFileSync(path.join(appRoot, 'certificate/trueleap_io.chained.crt')),
...
try {
    ...
    io.on('connection', socket => {
        ...
        socket.on('class-chat-event', msg => {
            socket.broadcast.emit('class-chat-event', msg);
        });

        socket.on('class-attendance-event', msg => {
            console.log('class-attendance-event');
            socket.broadcast.emit('class-attendance-event', msg);
        });

        socket.on('school-notification-event', msg => {
            socket.broadcast.emit('school-notification-event', msg);
        });

    });
} catch (ex) {
    logger.error("Socket io issue" + ex.stack);
}
//Socket End
